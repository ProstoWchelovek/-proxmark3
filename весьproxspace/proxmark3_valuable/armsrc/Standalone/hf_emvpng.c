...
//-----------------------------------------------------------------------------
// Project: EmvPng — EMV Emulator (fixed ARQC) – derived from MSDSal
//
// Copyright (C) 2023-2025 Davi Mikael C. R. (penegui)
// Modifications: EMV emulation with fixed ARQC for educational/lab use.
//
// Based on:
//   MSDSal by Salvador Mendoza (salmg.net)
//   Copyright (C) 2020 Salvador Mendoza
//   Copyright (C) Proxmark3 contributors — see AUTHORS.md
//
...
//-----------------------------------------------------------------------------
// Description:
//   Code derived from MSDSal and adapted for EMV testing with a fixed ARQC
//   in controlled environments. Educational use only.
//-----------------------------------------------------------------------------
// by Penegui
...
void ModInfo(void) {
    DbpString("  HF - Reading Visa cards & Emulating a Visa EMV Transaction(ISO14443) - (Penegui)");
}
...
* During this mode the Proxmark will behave as card, emulating a Visa EMV transaction
...
static uint8_t treatPDOL(const uint8_t *apdu) {
    ...
    for (uint8_t i = 1; i <= apdu[0]; i++) {          // Magic stuff, the generation order is important
        if (apdu[i] == 0x9F && apdu[i + 1] == 0x66) {   // Terminal Transaction Qualifiers
            ...
            plen += 2;
            i += 2;
        } else if (apdu[i] == 0x9F && apdu[i + 1] == 0x02) { // Authorized amount
            ppdol[plen] = 0x00;
            ppdol[plen + 1] = 0x00;
            ppdol[plen + 2] = 0x00;
            ppdol[plen + 3] = 0x00;
...
void RunMod(void) {
    StandAloneMode();
    DbpString(_YELLOW_(">>") "Reading Visa cards & Emulating a Visa EMV Transaction a.k.a. EmvPng Started " _YELLOW_("<<"));
        ...
        if (state == STATE_READ) {
            ...
            for (;;) {
                ...
                if (receivedCmd[0] == ISO14443A_CMD_REQA && len == 1) {
                    ...
                    // emulate a Visa EMV(Magnetic stripe data) card
                    if (receivedCmd[0] == 0x02 || receivedCmd[0] == 0x03) {
                        ...
                        if (receivedCmd[2] == 0xA4 && receivedCmd[6] == 0x32 && prevCmd == 0) {
                            uint8_t ppsea[72] = {
                                0x6F, 0x44, 0x84, 0x0E, 0x32, 0x50, 0x41, 0x59,
                                0x2E, 0x53, 0x59, 0x53, 0x2E, 0x44, 0x44, 0x46,
                                0x30, 0x31, 0xA5, 0x32, 0xBF, 0x0C, 0x2F, 0x61,
                                0x16, 0x4F, 0x07, 0xA0, 0x00, 0x00, 0x00, 0x03,
                            ...
                            uint8_t visauid_long[101] = {
                                0x6F, 0x61, 0x84, 0x07, 0xA0, 0x00, 0x00, 0x00,
                                0x03, 0x10, 0x10, 0xA5, 0x56, 0x50, 0x0C, 0x56,
                                0x49, 0x53, 0x41, 0x20, 0x43, 0x52, 0x45, 0x44,
                                0x49, 0x54, 0x4F, 0x87, 0x01, 0x01, 0x9F, 0x12,
                                ...
                                0x52, 0x09, 0x86, 0x00, 0x76, 0x30, 0x42, 0x03,
                                0x41, 0x08, 0x63, 0x90, 0x00
                            };
                            ...
                            uint8_t processing_long_last[8] = {0x77, 0x5f, 0x82, 0x02, 0x20, 0x00, 0x57, 0x13};
